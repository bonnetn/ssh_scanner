...
def get_logger():
    LOG = logging.getLogger("ssh_scanner")
...
def get_certificate(addr):
    LOG.debug("Checking cert {}".format(addr))
    ...
    if not crt:
        LOG.debug("No certificate found for {}".format(addr))
        return addr, None
    ...
    LOG.debug("Got certificate for {}".format(addr))
...
if __name__ == "__main__":
    ...
    for cert in certs_in_both_networks:
        ...
        LOG.info("{} = {} is SSHable in both networks.".format(addr1, addr2))
